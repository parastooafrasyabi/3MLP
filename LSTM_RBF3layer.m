...
% mean= unifrnd(lowerb,upperb, [n2_neurons n1_neurons]);
% sigma= unifrnd(lowerb,upperb, [n2_neurons 1]);
mean=0.5*ones(n2_neurons, n1_neurons);
sigma=ones(n2_neurons, 1);
W_2=unifrnd(lowerb,upperb,[l1_neurons n2_neurons]);
...
X1_z1=zeros((n1_neurons+n0_neurons+1),1);
    ...
    %% first LSTM layer
       % network input 
        ...
        net2=sum((h1' - mean).^2, 2);  % n2*1
        o2=exp(-0.5 * net2./(sigma.^2));  % n2*1
        ...
        f_derivative_c1=diag((1-c1.^2));
            ...
            delta_Wi_1=(eta_p  * e * W_2*(-1./sigma.^2)*(o2')*(h1'-mean))'*(o1.*(1-tansig(c1).^2))'*(f1.*f_derivative_i1_z1*c1_tilda_z1*X1_z1'+f_derivative_i1*c1_tilda*X1');
            delta_Wo_1=(eta_p  * e * W_2*(-1./sigma.^2)*(o2')*(h1'-mean))'*(tansig(c1))'* f_derivative_o1*X1';
            delta_Wf_1=(eta_p  * e * W_2*(-1./sigma.^2)*(o2')*(h1'-mean))'*(o1.*(1-tansig(c1).^2))'*(c1_z1'*f_derivative_f1)'*X1';
            delta_Wc_1=(eta_p  * e * W_2*(-1./sigma.^2)*(o2')*(h1'-mean))'*(o1.*(1-tansig(c1).^2))'*(f1.*f_derivative_c1_z1*i1_z1*X1_z1'+f_derivative_c1*i1*X1');
            delta_mean = eta_p*(e*W_2)'*((h1' - mean)'*(o2./(sigma.^2)))';
            delta_sigma=eta_p*(e*W_2)'.*sum((h1' - mean).^2, 2).*(1./(sigma.^3)).*o2; 
            delta_W_2= eta_p*e'*o2';
          %
%         % apdate weights
% 
